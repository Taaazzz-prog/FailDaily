...
create policy "Fail images are publicly accessible"
on "storage"."objects"
as permissive
for select
to public
using ((bucket_id = 'fails'::text));
...
create policy "Users can upload fail images"
on "storage"."objects"
as permissive
for insert
to public
with check (((bucket_id = 'fails'::text) AND ((auth.uid())::text = (storage.foldername(name))[1])));
