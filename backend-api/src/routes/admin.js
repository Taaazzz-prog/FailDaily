...
// GET /api/admin/rate-limit/stats - Statistiques rate limiting
router.get('/rate-limit/stats', authenticateToken, requireAdmin, async (req, res) => {
  try {
    const stats = {
      globalLimits: {
        general: { window: '15min', max: 5000 },
        login: { window: '15min', max: 5 },
        upload: { window: '15min', max: 10 },
        comments: { window: '1min', max: 30 },
        reactions: { window: '1min', max: 50 }
      },
      systemInfo: {
      ...
      message: 'Rate limiting actif - Protection DDoS par IP'
    };
    ...
    res.status(500).json({ 
      success: false, 
      message: 'Erreur récupération stats rate limiting' 
    });
  }
});

// GET /api/admin/rate-limit/info - Info publique rate limiting (sans auth)
router.get('/rate-limit/info', async (req, res) => {
  try {
    const info = {
      rateLimits: {
        ...
        reactions: '50 requests per minute'
      },
      protection: 'DDoS protection by IP address',
      environment: process.env.NODE_ENV || 'development',
      timestamp: new Date().toISOString()
    };
    ...
    res.status(500).json({ 
      success: false, 
      message: 'Erreur récupération info rate limiting' 
    });
  }
});

// GET /api/admin/rate-limit/blocked-ips - IPs bloquées
router.get('/rate-limit/blocked-ips', authenticateToken, requireAdmin, async (req, res) => {
  try {
    // Dans une vraie implémentation, on récupérerait les IPs depuis Redis/cache
    const blockedIps = [
      // Exemple de structure
      // { ip: '192.168.1.100', attempts: 150, blockedAt: '2025-09-05T14:10:00Z', reason: 'DDoS' }
    ];
    ...
    res.json({ 
      ...
      message: 'Liste des IPs actuellement bloquées'
    });
  } catch (error) {
    console.error('❌ Erreur blocked IPs:', error);
    res.status(500).json({ 
      success: false, 
      message: 'Erreur récupération IPs bloquées' 
    });
  }
});
